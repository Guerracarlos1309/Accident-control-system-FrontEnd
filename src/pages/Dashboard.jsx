  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const extList = inspectionsList.filter(i => i.type === "Extintor" || i.extinguisherInspection);
        const protList = inspectionsList.filter(i => i.type === "Proteccion" || i.protectionInspection);
        const vehList = inspectionsList.filter(i => i.type === "Vehiculo" || i.vehicleInspection);
        ...
        for (let i = 5; i >= 0; i--) {
          ...
          const count = accidentsList.filter(acc => {
            ...
            return accDate.getMonth() === d.getMonth() && accDate.getFullYear() === d.getFullYear();
          }).length;
        ...
        accidentsList.forEach(acc => {
          ...
          combinedEvents.push({
            ...
            time: acc.date ? new Date(acc.date).toLocaleDateString("es-VE", { day: "2-digit", month: "short" }) : "RECIENTE",
        ...
        inspectionsList.forEach(ins => {
          ...
          combinedEvents.push({
            ...
            time: ins.date ? new Date(ins.date).toLocaleDateString("es-VE", { day: "2-digit", month: "short" }) : "RECIENTE",
            path,
        ...
        const sorted = combinedEvents.sort((a, b) => b.rawDate - a.rawDate).slice(0, 4);
        setRecentActivity(sorted);

      } catch (err) {
        showNotification("Error al cargar indicadores dinámicos del Dashboard", "error");
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="relative flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-bg-surface backdrop-blur-xl p-8 md:p-10 rounded-[2rem] border border-border-main/50">
          <div className="space-y-2">
            <h2 className="text-2xl md:text-3xl font-black text-txt-main tracking-tight flex items-center gap-3 uppercase">
              Resumen de Gestión <Zap size={24} className="text-amber-400 fill-amber-400 animate-pulse" />
            </h2>
            <p className="text-txt-muted max-w-xl text-xs md:text-sm leading-relaxed">
              El panel de control ASHO reporta una <span className="text-emerald-500 font-black uppercase">Gestión Sincronizada</span>. 
              Los indicadores corresponden a los registros reales en tiempo real extraídos de la infraestructura del sistema.
            </p>
          </div>
          <button 
            onClick={() => navigate("/reports")}
            className="btn-primary px-8 h-14 cursor-pointer"
          >
            <Activity size={20} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 select-none font-black tracking-tighter">
        ...
        >
          ...
          <div>
            ...
            {loading ? (
              <Loader2 size={20} className="animate-spin text-txt-muted/30 mt-2" />
            ) : (
        ...
        >
          ...
          <div>
            <p className="text-[9px] font-black text-txt-muted uppercase tracking-[0.2em]">Incidentes Reportados</p>
            {loading ? (
              <Loader2 size={20} className="animate-spin text-txt-muted/30 mt-2" />
            ) : (
          ...
          <div className="glass-panel rounded-[2.5rem] p-8 md:p-10 border border-border-main/50 relative overflow-hidden group">
            <div className="flex justify-between items-center mb-10 relative z-10 select-none">
              <div className="space-y-1">
                <h4 className="text-lg md:text-xl font-black text-txt-main uppercase tracking-tight">Historial de Incidentes</h4>
                <p className="text-[9px] font-black text-txt-muted uppercase tracking-widest">Estadísticas reales de accidentabilidad de los últimos 6 meses</p>
              </div>
            ...
            {loading ? (
              ...
              <div className="h-64 flex items-end justify-between gap-4 px-4 relative z-10">
                {monthlyAccidents.map((m, idx) => {
                  ...
                  return (
                    <div key={idx} className="group flex-1 flex flex-col items-center gap-3">
                      ...
                      ></div>
                      <span className="text-[8px] font-black text-txt-muted uppercase tracking-widest">{m.label}</span>
                    </div>
                  );
                })}
              </div>
        ...
        <div className="glass-panel rounded-[2.5rem] p-8 md:p-10 border border-border-main/50 flex flex-col justify-between select-none">
          <div>
            <h4 className="text-xs font-black text-txt-main uppercase tracking-widest mb-8 pb-4 border-b border-border-main/50 flex items-center justify-between">
              ...
              <Award size={16} className="text-corpoelec-blue animate-spin-[20s] duration-1000" />
            </h4>
              ...
              <div className="space-y-6">
                {recentActivity.map((act) => (
                  ...
                  >
                    ...
                    <div className={`w-1.5 h-10 rounded-full transition-all group-hover:w-2 shrink-0 ${
                      act.type === 'accident' ? 'bg-corpoelec-red shadow-[0_0_10px_rgba(227,6,19,0.3)]' : 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.3)]'
                    }`} />
                    
                    <div className="flex-1 min-w-0 space-y-1">
                      <div className="flex justify-between items-start gap-2">
                        <p className="text-xs font-bold text-txt-main uppercase truncate group-hover:text-corpoelec-blue transition-colors leading-tight" title={act.title}>
                          {act.title}
                        </p>
