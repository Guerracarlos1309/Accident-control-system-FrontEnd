    ...
    <div className="flex items-end gap-1 h-24 w-full">
      {data.map((d, i) => {
        ...
        return (
          ...
          >
            <span className="text-[7px] font-black text-txt-muted/60 transition-colors group-hover:text-txt-main">
              {d.value > 0 ? d.value : ""}
            </span>
            <div
              ...
              style={{
                ...
                border: `1px solid ${d.value > 0 ? color + "60" : "rgba(255,255,255,0.08)"}`,
              }}
            />
            <span className="text-[6px] font-black text-txt-muted uppercase">
              {d.label}
            </span>
          </div>
        );
...
function StatMiniCard({
  ...
  const colors = {
    blue: {
      ...
      text: "text-corpoelec-blue",
      border: "hover:border-corpoelec-blue/30",
    },
    red: {
      bg: "bg-corpoelec-red/10",
      text: "text-corpoelec-red",
      border: "hover:border-corpoelec-red/30",
    },
    green: {
  ...
  return (
    <div
      className={`glass-panel p-5 rounded-[1.5rem] border border-border-main/50 ${c.border} transition-all duration-300 hover:scale-[1.02]`}
    >
      ...
      <p className="text-[8px] font-black text-txt-muted uppercase tracking-[0.2em] mb-1">
        {label}
      </p>
      {loading ? (
        <Loader2 size={16} className="animate-spin text-txt-muted/30 mt-1" />
      ) : (
        <h3
          className={`text-2xl font-black ${c.text} tracking-tighter tabular-nums`}
        >
          {value}
        </h3>
      )}
      {sub && (
        <p className="text-[8px] text-txt-muted mt-0.5 font-medium">{sub}</p>
      )}
    </div>
  );
}
...
function SectionDivider({
  ...
  borderColor = "from-corpoelec-blue/50",
}) {
  return (
    <div className="flex items-center gap-3">
      <div
        className={`h-px flex-1 bg-gradient-to-r ${borderColor} to-transparent`}
      />
      <span
        className={`text-[8px] font-black uppercase tracking-[0.3em] ${color} px-3 py-1 bg-bg-main/50 rounded-full border border-border-main/40`}
      >
        {label}
      </span>
      <div
        className={`h-px flex-1 bg-gradient-to-l ${borderColor} to-transparent`}
      />
    </div>
  );
}
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      setLoading(true);
      try {
        const [accRes, empRes, facRes, insRes, periodRes, vehRes] =
          await Promise.all([
            api.get("/accidents"),
            api.get("/employees"),
            api.get("/facilities"),
  ...
  useEffect(() => {
    ...
    const selectedYear = selectedPeriod
      ? parseInt(selectedPeriod.annuality)
      : null;
    ...
    const facsMap = {};
    filteredAccidents.forEach((a) => {
      const facName = a.facility?.name || "Ubicación Externa";
  ...
  const statsView = useMemo(() => {
    ...
    const totalRestDays = allInvolved.reduce(
      (s, e) => s + (parseInt(e.restDays) || 0),
      0,
    );
    const totalEmp = rawEmployees.length;
    const totalAcc = fAcc.length;
    const totalVeh = rawVehicles.length;
    const extL = fInsp.filter(
      (i) => i.type === "Extintor" || i.extinguisherInspection,
    );
    const protL = fInsp.filter(
      (i) => i.type === "Proteccion" || i.protectionInspection,
    );
    const vehL = fInsp.filter(
      (i) => i.type === "Vehiculo" || i.vehicleInspection,
    );
    ...
    return {
      ...
      totalRestDays,
      frecuencyIndex:
        totalEmp > 0 ? ((totalAcc / totalEmp) * 1000).toFixed(2) : "—",
      severityIndex:
        totalEmp > 0 ? ((totalRestDays / totalEmp) * 1000).toFixed(2) : "—",
      accRate: totalEmp > 0 ? ((totalAcc / totalEmp) * 100).toFixed(1) : "0.0",
      statusPending: fAcc.filter(
        (a) => a.processStatusId === 1 || !a.processStatusId,
      ).length,
      ...
      byMagnitude: count(fAcc, (a) => a.magnitude?.description || (a.magnitude?.value !== undefined ? `Magnitud ${a.magnitude.value}` : "Sin magnitud")),
      byFacility: count(fAcc, (a) => a.facility?.name || "Ubicación externa"),
      ...
      empByMgmt: count(
        rawEmployees,
        (e) => e.management?.name || "Sin gerencia",
      ),
      empByJob: count(
        rawEmployees,
        (e) => e.jobTitle?.name || "Sin cargo",
      ).slice(0, 8),
      vehByType: count(rawVehicles, (v) => v.type?.name || "Sin tipo"),
      vehByBrand: count(
        rawVehicles,
        (v) => v.model?.brand?.name || "Sin marca",
      ),
  ...
  const donutInspTypes = [
    {
      label: "Extintores",
      value: statsView.extL?.length || 0,
      color: "#10b981",
    },
    { label: "EPP/EPC", value: statsView.protL?.length || 0, color: "#3b82f6" },
    {
      label: "Vehículos",
      value: statsView.vehL?.length || 0,
      color: "#a855f7",
    },
  ];
        ...
        <div className="relative flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-bg-surface backdrop-blur-xl p-8 md:p-10 rounded-[2rem] border border-border-main/50">
          <div className="space-y-3">
            ...
            <p className="text-txt-muted max-w-xl text-xs leading-relaxed">
              {dashView === "resumen" ? (
                <>
                  Panel de control ASHO. Indicadores en{" "}
                ...
                <>
                  Análisis integral —{" "}
                  <span className="text-corpoelec-blue font-black">
                    accidentabilidad, inspecciones, talento humano y flota
                  </span>
                  .
                </>
              )}
            ...
            <div className="lg:col-span-2 space-y-6">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-[9px] font-black text-txt-muted uppercase tracking-widest">
                      {selectedPeriodId === "all"
                        ? "Últimos 6 meses"
                        : "Distribución mensual del año seleccionado"}
                    </p>
                  </div>
              ...
              <div className="glass-panel rounded-[2.5rem] p-8 md:p-10 border border-border-main/50 select-none">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                  <div className="space-y-1">
                    ...
                    <p className="text-[9px] font-black text-txt-muted uppercase tracking-widest">
                      Análisis cruzado por estado y tipos
                    </p>
                  </div>
                  <div className="flex bg-bg-main/40 p-1 rounded-xl border border-border-main/60 w-full sm:w-auto">
                    <button
                      onClick={() => setActiveTab("types")}
                      className={`flex-1 sm:flex-none text-[9px] font-black uppercase tracking-widest px-4 py-2 rounded-lg transition-all ${activeTab === "types" ? "bg-bg-surface text-corpoelec-blue shadow-sm border border-border-main/50" : "text-txt-muted hover:text-txt-main"}`}
                    >
                      Tipos de Suceso
                    </button>
                    <button
                      onClick={() => setActiveTab("facilities")}
                      className={`flex-1 sm:flex-none text-[9px] font-black uppercase tracking-widest px-4 py-2 rounded-lg transition-all ${activeTab === "facilities" ? "bg-bg-surface text-corpoelec-blue shadow-sm border border-border-main/50" : "text-txt-muted hover:text-txt-main"}`}
                    >
                      Por Instalación
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
                    <div className="md:col-span-1 space-y-4 bg-bg-main/20 p-6 rounded-3xl border border-border-main/40">
                      ...
                      {[
                        {
                          label: "Pendientes",
                          val: stats.statusPending,
                          color: "bg-amber-500",
                        },
                        {
                          label: "En Proceso",
                          val: stats.statusInProcess,
                          color: "bg-corpoelec-blue",
                        },
                        {
                          label: "Completados",
                          val: stats.statusCompleted,
                          color: "bg-green-500",
                        },
                      ].map((s, i) => (
                        <div key={i} className="space-y-1">
                          ...
                          <div className="h-2 w-full bg-bg-main/40 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${totalInvolvedStats > 0 ? (s.val / totalInvolvedStats) * 100 : 0}%`,
                              }}
                            />
                          </div>
                        </div>
              ...
              <div className="glass-panel rounded-[2rem] p-6 md:p-8 border border-border-main/50 select-none">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs font-bold uppercase tracking-wider">
                  <div
                    onClick={() => navigate("/inspections/extinguishers")}
                    className="bg-emerald-500/5 hover:bg-emerald-500/10 border border-emerald-500/20 p-4 rounded-2xl flex items-center justify-between transition-all cursor-pointer hover:scale-[1.03]"
                  >
                    <div className="space-y-1">
                  ...
                  <div
                    ...
                    className="bg-corpoelec-blue/5 hover:bg-corpoelec-blue/10 border border-corpoelec-blue/20 p-4 rounded-2xl flex items-center justify-between transition-all cursor-pointer hover:scale-[1.03]"
                  >
                    <div className="space-y-1">
                  ...
                  <div
                    ...
                    className="bg-purple-500/5 hover:bg-purple-500/10 border border-purple-500/20 p-4 rounded-2xl flex items-center justify-between transition-all cursor-pointer hover:scale-[1.03]"
                  >
                    <div className="space-y-1">
                ...
                {loading ? (
                  ...
                  <div className="space-y-6">
                    {recentActivity.map((act) => (
                      ...
                      >
                        <div
                          className={`w-1.5 h-10 rounded-full transition-all group-hover:w-2 shrink-0 ${act.type === "accident" ? "bg-corpoelec-red shadow-[0_0_10px_rgba(227,6,19,0.3)]" : "bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.3)]"}`}
                        />
      ...
      {dashView === "estadisticas" && (
        <div className="space-y-10">
          ...
          <SectionDivider
            ...
            borderColor="from-corpoelec-red/40"
          />

          {/* Tendencia mensual */}
          <div className="glass-panel rounded-[2rem] p-7 border border-border-main/50">
            <p className="text-[9px] font-black text-corpoelec-red uppercase tracking-widest mb-1">
              Tendencia Mensual
            </p>
            <h4 className="text-sm font-black text-txt-main uppercase tracking-tight mb-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              {
                title: "Por Tipo de Suceso",
                data: statsView.byType,
                color: "text-corpoelec-red",
              },
              {
                title: "Por Magnitud",
                data: statsView.byMagnitude,
                color: "text-amber-500",
              },
              {
                title: "Por Instalación",
                data: statsView.byFacility,
                color: "text-corpoelec-blue",
              },
            ].map((section, si) => (
              <div
                key={si}
                className="glass-panel rounded-[2rem] p-7 border border-border-main/50"
              >
                <p
                  className={`text-[8px] font-black uppercase tracking-widest mb-1 ${section.color}`}
                >
                  Accidentes
                </p>
                <h4 className="text-sm font-black text-txt-main uppercase tracking-tight mb-5">
                ...
                {loading ? (
                  <Loader2
                    size={18}
                    className="animate-spin text-txt-muted/30"
                  />
                ) : (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="glass-panel rounded-[2rem] p-7 border border-border-main/50">
              ...
              {loading ? (
                <Loader2 size={18} className="animate-spin text-txt-muted/30" />
              ) : (
                <div className="space-y-2.5">
                    ...
                    statsView.byManagement
                      ...
                      .map((t, i) => (
                        <HorizontalBar
                          ...
                          label={t.label}
                          value={t.value}
                          max={statsView.byManagement[0]?.value || 1}
                          color={CHART_COLORS[i % CHART_COLORS.length]}
                        />
                      ))
                  )}
            ...
            <div className="glass-panel rounded-[2rem] p-7 border border-border-main/50">
              <p className="text-[8px] font-black text-corpoelec-blue uppercase tracking-widest mb-1">
                Histórico
              </p>
              <h4 className="text-sm font-black text-txt-main uppercase tracking-tight mb-5">
                Evolución Anual de Accidentes
              </h4>
              {loading ? (
                <div className="h-24 flex items-center justify-center">
                  <Loader2
          ...
          {/* ── INSPECCIONES ─────────────────────────────────────────────── */}
          <SectionDivider
            label="Inspecciones de Seguridad"
            color="text-emerald-500"
            borderColor="from-emerald-500/40"
          />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="glass-panel rounded-[2rem] p-7 border border-border-main/50">
              ...
              <h4 className="text-sm font-black text-txt-main uppercase tracking-tight mb-4">
            ...
            <div className="lg:col-span-2 glass-panel rounded-[2rem] p-7 border border-border-main/50">
              ...
              <h4 className="text-sm font-black text-txt-main uppercase tracking-tight mb-5">
                Inspecciones Realizadas por Mes
              </h4>
              {loading ? (
                <div className="h-24 flex items-center justify-center">
                  <Loader2
                  ...
                  statsView.inspByFac
                    ...
                    .map((t, i) => (
                      <HorizontalBar
                        ...
                        label={t.label}
                        value={t.value}
                        max={statsView.inspByFac[0]?.value || 1}
                        color={CHART_COLORS[i % CHART_COLORS.length]}
                      />
                    ))
                )}
          ...
          <SectionDivider
            ...
            color="text-corpoelec-blue"
            borderColor="from-corpoelec-blue/40"
          />

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[
              {
                title: "Personal por Gerencia",
                data: statsView.empByMgmt,
                color: "text-corpoelec-blue",
              },
              {
                title: "Personal por Cargo",
                data: statsView.empByJob,
          ...
          <SectionDivider
            ...
            color="text-purple-500"
            borderColor="from-purple-500/40"
          />
            ...
            {[
              {
                ...
                data: statsView.vehByType,
                color: "text-purple-500",
              },
              {
                title: "Vehículos por Marca",
                data: statsView.vehByBrand,
                color: "text-cyan-500",
              },
            ].map((section, si) => (
              <div
