...
export default function ProtectionForm({ onCancel, onSuccess, initialData, inspectionsList = [] }) {
  const api = helpFetch();
    ...
    const fetchData = async () => {
      try {
        ...
        const mergedEquip = (catsRes || []).map(cat => {
          const config = (equipRes || []).find(eq => eq.categoryId === cat.id);
          ...
          return {
            ...
            name: cat.name,
            category: cat,
            description: config ? config.description : `CLASIF: ${defaultClassification} | MARCA: GENÉRICO | MODELO: N/A | COD: S/N`,
            lastUpdate: config ? config.lastUpdate : new Date().toISOString().split("T")[0]
          };
        });
        ...
        if (initialData) {
          ...
          const mappedEquip = mergedEquip.map(eq => {
            const detail = detailsList.find(d => d.categoryId === eq.categoryId) || {};
        ...
        showNotification("Error al cargar datos del formulario de inspección", "error");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!initialData?.id) {
      ...
      const selectedFac = lookups.facilities.find(f => f.id.toString() === formData.facilityId.toString());
      const facName = selectedFac ? selectedFac.name.trim().split(" ")[0].toUpperCase() : "SC";
      
      let nextSeq = 1;
      if (formData.facilityId) {
        const matchingInsps = (inspectionsList || []).filter(
          i => i.facilityId?.toString() === formData.facilityId.toString() && (i.type === "Proteccion" || i.protectionInspection)
        );
    ...
    const payload = {
      ...
      protectionData: {
        ...
        details: equipmentToInspect.map(eq => {
          ...
          const cleanComment = eq.commentText ? eq.commentText.trim().replace(/[|:]/g, "").slice(0, 100).toUpperCase() : "";
          // Compact pipeline format to fit into the 200 character observations string
          const serializedObservations = `B:${eq.buenos || 0}|M:${eq.malos || 0}|NE:${eq.noExisten || 0}|Obs:${cleanComment}`;
    ...
    try {
      ...
      if (res && !res.err) {
        showNotification(
          isEditing ? "Inspección de equipos actualizada con éxito" : "Inspección de equipos guardada con éxito", 
          "success"
        );
        if (onSuccess) onSuccess();
        onCancel();
      } else {
        showNotification(res.statusText || "Error al guardar inspección", "error");
      }
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 space-y-4 text-txt-muted">
        ...
        <p className="text-[10px] font-black tracking-[0.2em] uppercase">Preparando planilla de inspección...</p>
      </div>
    );
  }

  // Filter equipment list by Search term and active Tab (EPP/EPC)
  const filteredEquipToInspect = equipmentToInspect.map((eq, idx) => ({ ...eq, originalIndex: idx }))
    .filter(eq => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-h-[78vh] overflow-y-auto px-1 pr-2 text-txt-main no-scrollbar">
      ...
      <div className="bg-bg-main/5 p-5 rounded-3xl border border-border-main/50 space-y-4">
        <div className="flex items-center gap-2.5 pb-2 border-b border-border-main/45">
          ...
          <h4 className="text-[11px] font-black text-txt-muted uppercase tracking-[0.2em]">Datos de Localización y Control</h4>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black text-txt-muted uppercase tracking-[0.15em] ml-1 flex items-center gap-1 mt-1">
              <MapPin size={12} className="text-txt-muted" /> Centro de Trabajo / Sede *
            </label>
            <select 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1">
            <label className="text-[10px] font-black text-txt-muted uppercase tracking-[0.15em] ml-1 flex items-center gap-1 mt-1">
              <User size={12} className="text-txt-muted" /> Inspector Responsable *
            </label>
            <select 
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black text-txt-muted uppercase tracking-[0.15em] ml-1 flex items-center gap-1 mt-1">
              <Shield size={12} className="text-txt-muted" /> Diagnóstico del Lote *
            </label>
            <select 
                    ...
                    {filteredEquipToInspect.length === 0 ? (
                      <tr>
                        <td colSpan="7" className="text-center py-12 text-txt-muted">
                          <FolderOpen size={28} className="mx-auto mb-2 opacity-25 text-txt-muted" />
                          <p className="text-[10px] font-black uppercase tracking-wider">No se encontraron equipos registrados para esta clasificación</p>
                        </td>
                      </tr>
                    ) : (
                          ...
                          <tr key={eq.id} className="hover:bg-bg-main/5 transition-colors">
                            ...
                            <td className="px-4 py-3 bg-emerald-500/5">
                              <input
                                ...
                                onChange={(e) => handleQtyChange(idx, "buenos", e.target.value)}
                            ...
                            <td className="px-4 py-3 bg-corpoelec-red/5">
                              <input
                                ...
                                onChange={(e) => handleQtyChange(idx, "malos", e.target.value)}
                            ...
                            <td className="px-4 py-3 bg-amber-500/5">
                              <input
                                type="number"
                                min="0"
                                value={eq.noExisten === 0 ? "" : eq.noExisten}
                                onChange={(e) => handleQtyChange(idx, "noExisten", e.target.value)}
                                className="w-full rounded-xl px-2.5 py-2 text-center text-xs font-black bg-bg-surface border border-amber-500/30 text-amber-500 focus:border-amber-500 outline-none shadow-inner"
                              />
                            </td>
      ...
      <div className="bg-bg-main/5 p-5 rounded-3xl border border-border-main/50 space-y-4">
        ...
        <div className="space-y-1">
          <label className="text-[10px] font-black text-txt-muted uppercase tracking-[0.15em] ml-1">Observaciones Generales</label>
          <textarea 
            name="observations" rows="3" value={formData.observations} onChange={handleChange} 
      ...
      <div className="sticky bottom-0 bg-bg-surface pt-6 pb-2 border-t border-border-main flex justify-end gap-3 translate-y-2">
        ...
        <button 
          type="submit" disabled={isSubmitting || equipmentToInspect.length === 0} 
        ...
        >
          ...
          ) : initialData ? "Actualizar Informe" : "Guardar Informe Técnico"}
        </button>
      </div>
