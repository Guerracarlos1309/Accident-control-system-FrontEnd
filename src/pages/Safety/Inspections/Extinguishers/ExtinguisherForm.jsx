...
export default function ExtinguisherForm({
  ...
  const parseDetailObservations = (observationsStr, extinguisherNumber) => {
    ...
    if (!observationsStr.includes("|")) {
      if (observationsStr.startsWith("Mantenimiento: ")) {
        data.maintenancePart = observationsStr
    ...
    try {
      ...
      parts.forEach((part) => {
        ...
        if (cleanKey === "Code") data.code = cleanValue;
          ...
          data.impulseType = cleanValue.toLowerCase();
        else if (cleanKey === "Obs") data.observationsText = cleanValue;
        else if (cleanKey === "Maint")
          data.maintenancePart = cleanValue.toLowerCase();
        else if (cleanKey === "CK") {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        let defaultAgentTypeId =
          agentRes && agentRes.length > 0 ? agentRes[0].id : "";
  ...
  const getFacilityInitials = (facilityName) => {
    ...
    return facilityName
      ...
      .filter(
        (word) =>
          word.length > 0 &&
          !["de", "la", "el", "los", "las", "y"].includes(word.toLowerCase()),
      )
  ...
  const handleOpenAddExt = () => {
    if (!formData.facilityId) {
      showNotification(
        "Por favor, seleccione un Centro de Trabajo / Sede antes de vincular extintores",
        "warning",
      );
      return;
    }

    const selectedFacility = lookups.facilities.find(
      (f) => String(f.id) === String(formData.facilityId),
    );
    const initials = selectedFacility
      ? getFacilityInitials(selectedFacility.name)
      : "";
    ...
    let defaultAgentTypeId =
      lookups.agentTypes && lookups.agentTypes.length > 0
        ? lookups.agentTypes[0].id
        : "";
    setExtForm({
      code: defaultCode,
      physicalArea: "",
  ...
  const handleSaveExt = () => {
    ...
    if (rechargeDate > today) {
      showNotification(
        "La fecha de última recarga no puede ser una fecha futura",
        "error",
      );
      return;
    }
    ...
    if (extForm.expirationDate) {
      ...
      if (expDate <= rechargeDate) {
        showNotification(
          "La fecha de vencimiento debe ser posterior a la de última recarga",
          "error",
        );
        return;
      }
    ...
    if (selectedFacility) {
      ...
      if (initials) {
        ...
        if (!upperCode.includes(initials)) {
          showNotification(
            `El código del extintor debe incluir las iniciales de la sede (${initials})`,
            "error",
          );
          return;
        }
    ...
    if (extForm.generalStatus === "MANTENIMIENTO" && !extForm.maintenancePart) {
      showNotification(
        "Debe especificar la parte que requiere mantenimiento",
        "error",
      );
      return;
    }
    ...
    showNotification(
      editingExtIndex !== null
        ? "Extintor actualizado en la lista"
        : "Extintor agregado a la lista",
      "success",
    );
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.date) {
      ...
      if (auditDate > today) {
        showNotification(
          "La fecha de auditoría no puede ser una fecha futura",
          "error",
        );
        return;
      }
    ...
    if (formData.extinguishers.length === 0) {
      showNotification(
        "Debe agregar al menos un extintor a la lista de inspección",
        "error",
      );
      return;
    }
    ...
    if (!initialData?.id) {
      ...
      const facilityInitials = selectedFacility
        ? getFacilityInitials(selectedFacility.name)
        : "";
      ...
      const seqNumbers = (inspectionsList || [])
        ...
        .filter(
          (num) =>
            typeof num === "string" &&
            num.startsWith(`EXT-${year}-${facilityInitials}-`),
        )
        .map((num) => {
      ...
      finalInspectionNumber =
        `EXT-${year}-${facilityInitials}-${String(nextSeq).padStart(3, "0")}-${extCount}`.slice(
          0,
          20,
        );
    }
    ...
    const payload = {
      ...
      extinguisherData: {
        extinguisherCode:
          formData.extinguishers[0]?.code.toUpperCase() || "MULTIPLE",
    ...
    try {
      ...
      if (res && !res.err) {
        showNotification(
          isEditing
            ? "Auditoría de extintores actualizada correctamente"
            : "Auditoría de extintores guardada correctamente",
          "success",
        );
        ...
        showNotification(
          res.statusText || "Error al registrar auditoría",
          "error",
        );
      }
    } catch (error) {
    ...
    <div className="flex items-center justify-between p-4 bg-bg-main/5 rounded-2xl border border-border-main/40 hover:border-corpoelec-blue/30 transition-colors">
      ...
      <div className="flex bg-bg-surface rounded-xl p-1 border border-border-main">
        ...
        <button
          ...
          className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all cursor-pointer ${
            !extForm[field]
              ? "bg-corpoelec-red text-white shadow-md shadow-corpoelec-red/15"
              : "text-txt-muted hover:text-txt-main"
          }`}
        >
          Malo
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="bg-bg-surface w-full max-w-3xl rounded-[2.5rem] border border-border-main shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
            <div className="p-6 border-b border-border-main bg-bg-main/5 flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-xs font-black uppercase text-txt-main tracking-widest">
                  {editingExtIndex !== null
                    ? "Editar Ficha de Extintor"
                    : "Agregar Ficha de Extintor"}
                </h4>
              </div>
              <button
                type="button"
            ...
            <div className="p-6 overflow-y-auto space-y-6 no-scrollbar flex-1">
              ...
              <div className="bg-bg-main/5 p-5 rounded-3xl border border-border-main/50 space-y-4">
                <h5 className="text-[11px] font-black text-txt-muted uppercase tracking-[0.2em] pb-1 border-b border-border-main/30">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      className="input-field h-11 font-bold text-corpoelec-blue border border-border-main"
                      placeholder="Ej: EXT-005"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[11px] font-black text-txt-muted uppercase tracking-[0.2em] ml-1">
                    ...
                    >
                      ...
                      <option value="Pasillo Secundario">
                        Pasillo Secundario
                      </option>
                      <option value="Almacén / Depósito">
                        Almacén / Depósito
                      </option>
                      <option value="Cuarto Eléctrico">Cuarto Eléctrico</option>
                      <option value="Sala de Servidores">
                        Sala de Servidores
                      </option>
                      <option value="Taller / Área Técnica">
                        Taller / Área Técnica
                      </option>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      required
                      value={extForm.lastRechargeDate}
                      onChange={handleExtFormChange}
                      className="input-field h-11 border border-border-main"
                    />
                  </div>
              ...
              <div className="bg-bg-main/5 p-5 rounded-3xl border border-border-main/50 space-y-4">
                <h5 className="text-[11px] font-black text-txt-muted uppercase tracking-[0.2em] pb-1 border-b border-border-main/30">
                  2. Inspección Visual del Puesto
                </h5>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  />
                  <ChecklistItem
                    label="Señalización Visible"
                    field="signageOk"
                  />
                  <ChecklistItem label="Área Demarcada" field="demarcationOk" />
                  <ChecklistItem
                    label="Acceso Libre de Obstrucciones"
                    field="accessOk"
                  />
                  <div className="sm:col-span-2">
                    <ChecklistItem
                      label="Funcionamiento de Manómetro / Precinto de Seguridad"
                      field="operationOk"
                    />
                  </div>
                </div>
              ...
              <div className="bg-bg-main/5 p-5 rounded-3xl border border-border-main/50 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    <label className="text-[11px] font-black text-txt-muted uppercase tracking-[0.2em] ml-1">
                      Parte que requiere mantenimiento
                    </label>
                    <select
                    ...
                    >
                      ...
                      <option value="manometro">
                        Manómetro (Baja/Alta Presión)
                      </option>
                      <option value="precinto">
                        Precinto de Seguridad (Faltante/Roto)
                      </option>
                      <option value="cilindro">
                        Cilindro (Oxidación/Golpe)
                      </option>
                      <option value="valvula">
                        Válvula / Gatillo de Accionamiento
                      </option>
                    </select>
                  </div>
                  <div className="space-y-1 sm:col-span-2">
                    ...
                    <input
                      ...
                      name="observations"
                      maxLength={30}
                      value={extForm.observations}
                      onChange={handleExtFormChange}
                      className="input-field h-11 border border-border-main placeholder:text-txt-muted/30"
                      placeholder="Escriba notas del cilindro..."
                    />
                  </div>
                </div>
              </div>
      ...
      <form
        ...
        className="space-y-6 max-h-[75vh] overflow-y-auto px-1 pr-2 no-scrollbar"
      >
        ...
        <div className="bg-bg-main/5 p-5 rounded-3xl border border-border-main/50 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              <label className="text-[11px] font-black text-txt-muted uppercase tracking-[0.2em] ml-1">
                Resultado de Inspección General *
              </label>
              <select
                name="statusId"
                required
          ...
          <div className="space-y-1">
            ...
            <textarea
              ...
              onChange={handleChange}
              className="input-field py-3 resize-none min-h-[70px] border border-border-main placeholder:text-txt-muted/30"
              placeholder="Escriba comentarios sobre el estado de la sede..."
            />
          </div>
        </div>
        ...
        <div className="bg-bg-main/5 p-5 rounded-3xl border border-border-main/50 space-y-4">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-2 border-b border-border-main/50">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-[11px] font-black text-txt-muted uppercase tracking-[0.2em]">
                Equipos Evaluados ({formData.extinguishers.length})
              </h4>
            </div>
            <button
              type="button"
          ...
          {formData.extinguishers.length === 0 ? (
            <div className="p-16 border-2 border-dashed border-border-main/60 rounded-[2rem] text-center bg-bg-main/5 flex flex-col items-center">
              ...
              <p className="text-txt-muted font-black uppercase tracking-widest text-[9px]">
                No hay extintores agregados a este reporte.
              </p>
              <p className="text-txt-muted/50 text-[8px] font-bold mt-1.5 uppercase">
                Presione "+ Vincular Extintor" para evaluar un equipo en las
                instalaciones.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {formData.extinguishers.map((ext, index) => {
                const agentName =
                  lookups.agentTypes.find(
                    (a) => String(a.id) === String(ext.agentTypeId),
                  )?.name || "Extintor";
                return (
                  <div
                    key={index}
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <span className="text-lg font-black text-txt-main tracking-tight uppercase">
                          {ext.code}
                        </span>
                    ...
                    <div className="space-y-2 border-t border-border-main/50 pt-3 text-[10px] font-bold text-txt-sub">
                      <div className="flex justify-between">
                        <span className="text-txt-muted font-black uppercase text-[8px] tracking-wider">
                          Ubicación física:
                        </span>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span>
                          {ext.lastRechargeDate} a {ext.expirationDate}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        ...
                        <span
                          className={`px-2 py-0.5 rounded text-[8px] font-black ${
                            ext.generalStatus === "OPERATIVO"
                              ? "bg-green-500/10 text-green-500 border border-green-500/20"
                              : ext.generalStatus === "MANTENIMIENTO"
