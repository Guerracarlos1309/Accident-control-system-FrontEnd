...
export default function AccidentDetails({ accident }) {
  ...
  const SectionTitle = ({ icon: Icon, title, color = "text-corpoelec-blue" }) => (
  ...
  const DataRow = ({ label, value }) => (
    <div className="py-2 border-b border-border-main/50 flex justify-between items-center gap-4">
      ...
      <span className="text-xs font-bold text-txt-main text-right">{value || "---"}</span>
    </div>
  );
  ...
  return (
    <div className="space-y-8 max-h-[70vh] overflow-y-auto pr-4 no-scrollbar">
      ...
      <div className="p-6 bg-corpoelec-blue/5 rounded-3xl border border-corpoelec-blue/10 flex flex-col md:flex-row justify-between gap-6">
        ...
        <div className="flex flex-col items-end gap-2">
            <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${
                accident.status === 1 ? "bg-green-500/10 text-green-500 border-green-500/20" : "bg-corpoelec-red/10 text-corpoelec-red border-corpoelec-red/20"
            }`}>
                {accident.status === 1 ? "Registro Activo" : "Registro Archivado"}
            </span>
            <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${
                accident.processStatusId === 3 ? "bg-green-500/10 text-green-500 border-green-500/20" : 
                accident.processStatusId === 2 ? "bg-corpoelec-blue/10 text-corpoelec-blue border-corpoelec-blue/20" : 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          <section>
            ...
            <div className="glass-panel p-5 rounded-2xl space-y-1">
              ...
              />
              <DataRow label="Tipo Sede" value={accident.facility?.installationType?.name || "N/A (Ubicación Externa)"} />
              <DataRow label="Detalle Dirección" value={accident.customAddressDetails || accident.facility?.location?.name} />
            </div>
          </section>
          ...
          <section>
            ...
            <div className="glass-panel p-5 rounded-2xl space-y-1">
              ...
              <DataRow label="Gerencia Involucrada" value={accident.management?.name} />
              <DataRow label="Tipo de Accidente" value={accident.type?.name} />
              <DataRow label="Año / Periodo" value={accident.period?.annuality} />
              <DataRow label="Agente de Daño" value={accident.damageAgent?.name} />
              <DataRow label="Tipo de Contacto" value={accident.contactType?.name} />
            </div>
          </section>
        </div>
        ...
        <div className="space-y-6">
          <section>
            <SectionTitle icon={User} title="Personal Involucrado" color="text-corpoelec-red" />
            <div className="space-y-3">
              {accident.involvedEmployees?.length > 0 ? (
                accident.involvedEmployees.map((inv, idx) => (
                  <div key={idx} className="p-4 bg-bg-main/20 rounded-2xl border border-border-main/50 flex items-center justify-between group">
                    <div className="flex items-center gap-3">
                      <div className="h-10 w-10 bg-corpoelec-red/10 text-corpoelec-red rounded-full flex items-center justify-center font-black text-xs">
                        {inv.employee?.firstName?.[0]}{inv.employee?.lastName?.[0]}
                      </div>
                      <div>
                        <p className="text-xs font-bold text-txt-main">{inv.employee?.firstName} {inv.employee?.lastName}</p>
                        <p className="text-[9px] font-black text-txt-muted uppercase">CI: {inv.employee?.idCard || inv.employee?.id_card}</p>
                      </div>
                    </div>
                    <button
                ...
                <p className="text-xs text-txt-muted italic ml-1">No se registró personal específico.</p>
              )}
            </div>
          </section>

          <section>
            <SectionTitle icon={Users} title="Testigos" color="text-amber-500" />
            <div className="space-y-3">
              {accident.witnesses?.length > 0 ? (
                accident.witnesses.map((witness, idx) => (
                  <div key={idx} className="p-4 bg-amber-500/5 rounded-2xl border border-amber-500/10 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="h-8 w-8 bg-amber-500/10 text-amber-500 rounded-lg flex items-center justify-center font-black text-[10px]">
                        W{idx + 1}
                      </div>
                      <div>
                        <p className="text-[11px] font-bold text-txt-main">{witness.name}</p>
                        <p className="text-[9px] font-black text-txt-muted uppercase">CI: {witness.idCard || "---"} | Tel: {witness.phone || "---"}</p>
                      </div>
                    </div>
                  </div>
                ))
              ) : (
                <p className="text-xs text-txt-muted italic ml-1">No se registraron testigos.</p>
              )}
            </div>
          </section>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <section>
            <SectionTitle icon={Stethoscope} title="Diagnóstico / Salud" color="text-green-500" />
            <div className="glass-panel p-5 rounded-2xl space-y-1">
               <DataRow label="Centro Médico" value={accident.medicalCenterName} />
               <div className="mt-4">
                  <p className="text-[10px] font-black text-txt-muted uppercase tracking-widest mb-2">Observaciones Médicas</p>
                  <p className="text-xs text-txt-main leading-relaxed bg-bg-main/30 p-3 rounded-xl italic">
                    "{accident.medicalObservations || "Sin observaciones registradas."}"
                  </p>
               </div>
            </div>
          </section>
          ...
          <section>
            <SectionTitle icon={FileText} title="Checklist Documental" color="text-corpoelec-blue" />
            <div className="glass-panel p-5 rounded-2xl grid grid-cols-1 gap-2">
               {accident.documentsCheck?.length > 0 ? (
                 accident.documentsCheck.map((doc, idx) => (
                   <div key={idx} className="flex items-center gap-2 text-xs font-bold text-txt-main">
                 ...
                 <p className="text-xs text-txt-muted italic">No hay documentos marcados.</p>
               )}
            </div>
          </section>
      </div>
      ...
      {/* Descripción Técnica */}
      <section>
        <SectionTitle icon={FileText} title="Descripción Técnica de los Hechos" />
                 ...
                 <div className="space-y-4">
                    <div className="p-4 bg-bg-main/50 rounded-2xl border border-border-main/50">
                       <div className="flex items-center gap-3 mb-3">
                          ...
                          <span className="text-[10px] font-black text-txt-muted uppercase tracking-widest">Datos Institucionales</span>
                       </div>
                       <div className="space-y-3">
                          <div>
                             <p className="text-[9px] font-black text-txt-muted uppercase mb-0.5">Gerencia</p>
                             <p className="text-xs font-bold text-txt-main">{showEmployeeCard.management?.name || "No especificada"}</p>
                          </div>
                          <div>
                             <p className="text-[9px] font-black text-txt-muted uppercase mb-0.5">Ocupación Específica</p>
                             <p className="text-xs font-bold text-txt-main">{showEmployeeCard.occupation?.name || "No especificado"}</p>
                          </div>
                       </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                       <div className="p-4 bg-bg-main/50 rounded-2xl border border-border-main/50 text-center">
                          <p className="text-[9px] font-black text-txt-muted uppercase mb-1">Cédula</p>
                          <p className="text-xs font-black text-corpoelec-blue">{showEmployeeCard.idCard || showEmployeeCard.id_card}</p>
                       </div>
                       <div className="p-4 bg-bg-main/50 rounded-2xl border border-border-main/50 text-center">
                          <p className="text-[9px] font-black text-txt-muted uppercase mb-1">Personal</p>
                          <p className="text-xs font-black text-corpoelec-blue">{showEmployeeCard.personalNumber}</p>
                       </div>
                    </div>
                 </div>
              </div>
