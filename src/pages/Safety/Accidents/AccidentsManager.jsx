...
import { Plus, AlertCircle, FileText, Calendar, MapPin, Loader2, User, Clock, LayoutGrid, List } from "lucide-react";
...
export default function AccidentsManager() {
  ...
  const handleSaveAccident = async (data) => {
    try {
      ...
      if (res && !res.err) {
        ...
        showNotification(res.message || "Error al registrar accidente", "error");
      }
    } catch (error) {
  ...
  const handleToggleStatus = async (acc) => {
    try {
      ...
      const res = await api.put(`/accidents/${acc.id}`, { body: { status: newStatus } });
      if (res && !res.err) {
        showNotification(`Accidente ${newStatus === 1 ? 'activado' : 'archivado'}`, "success");
        fetchAccidents();
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-24 space-y-4 glass-panel rounded-3xl">
          ...
          <p className="text-txt-muted font-black tracking-widest uppercase text-[10px]">Consultando registros...</p>
        </div>
      ) : accidents.length === 0 ? (
        <div className="glass-panel rounded-3xl p-12 text-center border border-border-main/50">
          ...
          <h3 className="text-sm font-black text-txt-main uppercase tracking-widest">Sin Accidentes Registrados</h3>
          <p className="text-txt-muted mt-2 text-xs italic max-w-xs mx-auto">No se han encontrado reportes de investigación en el sistema.</p>
        </div>
      ) : (
        <>
          {viewMode === "grid" ? (
                ...
                <div key={acc.id} className={`glass-panel p-6 rounded-3xl border ${acc.status === 1 ? 'border-border-main/50' : 'border-corpoelec-red/20 opacity-75'} hover:border-corpoelec-blue/30 transition-all group relative overflow-hidden`}>
                  ...
                  <div className="flex justify-between items-start mb-6 relative z-10">
                    <div className="flex items-center gap-3">
                        <div className={`p-3 ${acc.status === 1 ? 'bg-corpoelec-red/10 text-corpoelec-red' : 'bg-txt-muted/10 text-txt-muted'} rounded-2xl`}>
                          <AlertCircle size={20} />
                        </div>
                        <div>
                            <span className="text-[10px] font-black text-txt-muted uppercase tracking-widest block">ID #{acc.id}</span>
                            <span className="text-[10px] font-black text-corpoelec-blue uppercase tracking-widest">Año: {acc.period?.annuality}</span>
                        </div>
                    </div>
                  ...
                  <div className="space-y-4 relative z-10">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-[10px] font-black text-txt-muted uppercase tracking-widest leading-none mb-1">Afectado:</p>
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-border-main/50">
                        <div className="flex flex-col gap-2">
                           <div className="flex items-center gap-2">
                              <span className={`h-2 w-2 rounded-full ${acc.status === 1 ? 'bg-green-500 animate-pulse' : 'bg-corpoelec-red'}`}></span>
                              <span className="text-[9px] font-black text-txt-muted uppercase tracking-widest">{acc.status === 1 ? 'Activo' : 'Archivado'}</span>
                           </div>
                           <div className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest inline-flex items-center gap-2 ${
                              acc.processStatusId === 3 ? 'bg-green-500/10 text-green-500' : 
                              acc.processStatusId === 2 ? 'bg-corpoelec-blue/10 text-corpoelec-blue' : 
                              'bg-amber-500/10 text-amber-500'
                           }`}>
                              <span className={`h-1.5 w-1.5 rounded-full ${
                                 acc.processStatusId === 3 ? 'bg-green-500' : 
                                 acc.processStatusId === 2 ? 'bg-corpoelec-blue' : 
                                 'bg-amber-500'
                              }`}></span>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-bg-main/30 border-b border-border-main">
                    <th className="p-5 text-[10px] font-black text-txt-muted uppercase tracking-widest">Estado</th>
                    <th className="p-5 text-[10px] font-black text-txt-muted uppercase tracking-widest">ID</th>
                    <th className="p-5 text-[10px] font-black text-txt-muted uppercase tracking-widest">Afectado</th>
                    <th className="p-5 text-[10px] font-black text-txt-muted uppercase tracking-widest">Año</th>
                    <th className="p-5 text-[10px] font-black text-txt-muted uppercase tracking-widest text-center">Acciones</th>
                  </tr>
                </thead>
                <tbody>
                  {accidents.filter(a => a.status === filterStatus).map((acc) => (
                    <tr key={acc.id} className={`border-b border-border-main/50 hover:bg-bg-main/20 transition-colors ${acc.status === 0 ? 'opacity-60 bg-bg-main/10' : ''}`}>
                      <td className="p-5">
                        <div className="flex flex-col gap-1">
                            <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest w-fit ${acc.status === 1 ? 'bg-green-500/10 text-green-500' : 'bg-corpoelec-red/10 text-corpoelec-red'}`}>
                                {acc.status === 1 ? 'Activo' : 'Archivado'}
                            </span>
                            <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest w-fit ${
                                acc.processStatusId === 3 ? 'bg-green-500/20 text-green-600' : 
                                acc.processStatusId === 2 ? 'bg-corpoelec-blue/20 text-corpoelec-blue' : 
                      ...
                      <td className="p-5 text-xs font-bold text-txt-main">#{acc.id}</td>
                      <td className="p-5">
      ...
      <Modal 
        ...
        title={selectedAccident ? "Editar Investigación" : "Nueva Investigación"}
        maxWidth="max-w-4xl"
      ...
      >
        <AccidentForm 
          ...
          onSubmit={async (data) => {
            if (selectedAccident) {
              try {
                const res = await api.put(`/accidents/${selectedAccident.id}`, { body: data });
                if (res && !res.err) {
                  showNotification("Investigación actualizada exitosamente", "success");
                  setIsModalOpen(false);
                  fetchAccidents();
                } else {
