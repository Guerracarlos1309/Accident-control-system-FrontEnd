...
export default function VehicleRegistryForm({
  onCancel,
  initialData = null,
  onSuccess,
}) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!/^[A-Za-z0-9-]+$/.test(plateVal)) {
      showNotification(
        "La placa solo puede contener letras, números y guiones (sin espacios)",
        "error",
      );
      return;
    }
    ...
    if (isNaN(yearVal) || yearVal < 1900 || yearVal > currentYear + 1) {
      showNotification(
        `El año de fabricación debe estar entre 1900 y ${currentYear + 1}`,
        "error",
      );
      return;
    }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="col-span-1 border-b border-border-main pb-4 md:col-span-2">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h4 className="text-[11px] font-bold text-txt-muted uppercase tracking-widest">
          ...
          {existingImages.length > 0 || imagePreviews.length > 0 ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <button
                      ...
                      className="bg-corpoelec-red/20 text-corpoelec-red hover:bg-corpoelec-red hover:text-white p-2 rounded-full transition-colors"
                    >
                      <X size={16} />
                    </button>
                  </div>
              ...
              {imagePreviews.map((preview, idx) => (
                <div
                  key={idx}
                  className="relative aspect-square rounded-xl overflow-hidden group border-2 border-dashed border-corpoelec-blue/50"
                >
                  <img
                    src={preview}
                    alt="Preview"
                  ...
                  />
                  <div className="absolute top-1 right-1 bg-corpoelec-blue text-white text-[9px] font-bold px-1.5 py-0.5 rounded uppercase">
                    Nueva
                  </div>
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <button
