...
import { Plus, Search, Edit2, Trash2, Filter, LayoutGrid, List, Loader2, Eye, AlertTriangle, CheckCircle2 } from "lucide-react";
...
export default function MasterEntityManager({ 
  ...
  const handleSubmit = async (e, providedData = null) => {
    ...
    try {
      ...
      if (res && !res.err) {
        ...
        showNotification(res.message || res.statusText || "Error en la operación", "error");
      }
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res && !res.err) {
        showNotification(`${entityName} ${deleteMode === 'hard' ? 'eliminado permanentemente' : 'desactivado'}`, "success");
        fetchData();
  ...
  return (
    <div className="space-y-6">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-1 opacity-100 lg:opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button 
                          ...
                          title={deleteMode === 'hard' ? "Eliminar Permanentemente" : "Desactivar"}
                        >
                          <Trash2 size={16} />
                        </button>
                      </div>
          ...
          {filteredData.length > 0 ? filteredData.map((item, idx) => (
            <div key={idx} className="glass-panel p-6 rounded-3xl border border-border-main/50 transition-colors group relative">
              <div className="absolute top-4 right-4 flex gap-1">
                {allowReactivate && (
                  <button onClick={() => handleReactivate(item)} className="p-2 bg-bg-surface rounded-xl text-green-500 border border-border-main/50 hover:bg-green-500/10" title="Reactivar"><CheckCircle2 size={14} /></button>
                )}
                {ViewComponent && (
                  <button onClick={() => handleOpenView(item)} className="p-2 bg-bg-surface rounded-xl text-txt-muted border border-border-main/50 hover:bg-corpoelec-blue/10 hover:text-corpoelec-blue" title="Ver detalles"><Eye size={14} /></button>
                )}
                <button onClick={() => handleOpenModal(item)} className="p-2 bg-bg-surface rounded-xl text-corpoelec-blue border border-border-main/50 hover:bg-corpoelec-blue/10" title="Editar"><Edit2 size={14} /></button>
                <button 
                  onClick={() => handleOpenDelete(item)}
      ...
      <Modal
        ...
        title={deleteMode === 'hard' ? "Confirmar Eliminación Permanente" : "Confirmar Desactivación"}
        maxWidth="max-w-md"
      >
        <div className="text-center space-y-4 py-4">
          <div className={`h-20 w-20 ${deleteMode === 'hard' ? 'bg-red-500/10 text-red-500' : 'bg-corpoelec-red/10 text-corpoelec-red'} rounded-full flex items-center justify-center mx-auto border-4 border-current/5`}>
            <AlertTriangle size={40} />
          </div>
          <div>
            <h3 className="text-lg font-bold text-txt-main uppercase tracking-tighter">
              {deleteMode === 'hard' ? "¿Eliminar para siempre?" : "¿Estás seguro?"}
            </h3>
            <p className="text-sm text-txt-muted mt-2 px-4 italic">
              ...
              <span className="font-bold text-txt-main not-italic">"{itemToDelete?.name || itemToDelete?.firstName || itemToDelete?.[idField] || "este registro"}"</span>.
          ...
          <div className="pt-8 flex flex-col gap-2">
            ...
            >
              <CheckCircle2 size={16} />
              {deleteMode === 'hard' ? "Eliminar Definitivamente" : "Confirmar Desactivación"}
            </button>
            <button 
