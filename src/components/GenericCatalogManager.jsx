            ...
            <tbody className="divide-y divide-border-main/30">
              {filteredData.length > 0 ? (
                filteredData.map((item, idx) => (
                  <tr
                    key={idx}
                    className="hover:bg-bg-primary/5 transition-colors group"
                  >
                    {fields.map((f) => (
                      <td
                        key={f.name}
                        className="px-6 py-4 text-sm font-bold text-txt-sub"
                      >
                        {item[f.name] || (
                ...
                <tr>
                  <td
                    ...
                    className="px-6 py-24 text-center text-txt-muted"
                  >
            ...
            {fields.map((f) => (
              <div key={f.name} className="space-y-2">
                <label className="text-xs font-black text-txt-sub uppercase tracking-widest ml-1">
                  {f.label}{" "}
                  {f.required && <span className="text-corpoelec-red">*</span>}
                </label>
                {f.type === "textarea" ? (
                  <textarea
                    name={f.name}
